...
(async () => {
  ...
  // Tags created by tag-specific specs. FK CASCADE on the junction
  // tables (lead_tags / account_tags / contact_tags /
  // opportunity_tags / task_tags) drops dependent rows automatically
  // when the parent tag is removed.
  await clean(
    "tags",
    sql`DELETE FROM tags WHERE name ILIKE ${pattern}`,
  );
