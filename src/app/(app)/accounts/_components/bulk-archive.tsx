 ...
 * /accounts desktop table. Context is consumed by the toolbar
...
export function BulkArchiveProvider({ children }: { children: React.ReactNode }) {
  ...
  const archive = useCallback(() => {
    ...
    if (
      !confirm(
        `Archive ${selected.size} ${selected.size === 1 ? "account" : "accounts"}? Linked contacts and opportunities remain visible.`,
      )
    ) {
      return;
    }
    startTransition(async () => {
      ...
      if (denied > 0) {
        ...
        toast.success(
          `Archived ${archived} ${archived === 1 ? "account" : "accounts"}.`,
        );
      }
      setSelected(new Set());
      router.refresh();
