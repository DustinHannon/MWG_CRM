...
import { setAccountAdhocColumnsAction } from "../view-actions";
import {
...
/**
 * DnD column reorder on /accounts. Wraps the server-rendered
 ...
 * via `setAccountAdhocColumnsAction`.
 */
export function SortableAccountsHeaders({
  initialColumns,
  activeViewId,
}: {
  initialColumns: AccountColumnKey[];
  /** "saved:<uuid>" or "builtin:<key>" — passed through for future
   * saved-view persistence. */
  activeViewId: string;
}) {
  const [columns, setColumns] = useState<AccountColumnKey[]>(initialColumns);
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    const oldIndex = columns.indexOf(active.id as AccountColumnKey);
    const newIndex = columns.indexOf(over.id as AccountColumnKey);
    ...
    startTransition(async () => {
      const res = await setAccountAdhocColumnsAction(fd);
      if (!res.ok) {
        setColumns(columns);
...
function SortableHeader({
  ...
  return (
    <th
      ...
      {...listeners}
    >
      {AVAILABLE_ACCOUNT_COLUMNS.find((col) => col.key === id)?.label ?? id}
    </th>
  );
}
