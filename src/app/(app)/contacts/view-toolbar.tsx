...
/**
 * Top-of-page toolbar for /contacts. View selector (built-in + saved)
...
function SaveViewDialog({
  ...
  defaultColumns: ContactColumnKey[];
  activeViewId: string;
  search: string;
  onClose: () => void;
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    const res = await createContactViewAction(fd);
    setSubmitting(false);
    if (!res.ok) {
        ...
        <form onSubmit={onSubmit} className="mt-4 space-y-3">
          <label className="block text-xs uppercase tracking-wide text-muted-foreground">
            ...
            <input
              ...
              placeholder="e.g. Atlanta benefits contacts"
            />
          </label>
