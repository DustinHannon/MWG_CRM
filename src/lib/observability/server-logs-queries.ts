...
const RANGE_TO_INTERVAL: Record<ServerLogsRange, string> = {
  "1h": "INTERVAL 1 HOUR",
  "6h": "INTERVAL 6 HOUR",
 ...
 * recommendation.
 */
function buildFromClause(range: ServerLogsRange): string {
...
export async function getErrorPatterns(
  ...
  return queryBetterStack<ErrorPatternRow>({
    query,
    cacheKey: `server-logs.error-patterns.${range}`,
...
export async function getRequestVolume(
  ...
  return queryBetterStack<RequestVolumeRow>({
    query,
    cacheKey: `server-logs.request-volume.${range}`,
    auditFamily: "observability.server_logs",
  });
}
...
export async function getStatusDistribution(
  ...
  return queryBetterStack<StatusDistributionRow>({
    query,
    cacheKey: `server-logs.status-distribution.${range}`,
    auditFamily: "observability.server_logs",
  });
}
...
export async function getDeployTimeline(): Promise<DeployTimelineRow[]> {
  ...
  return queryBetterStack<DeployTimelineRow>({
    query,
    cacheKey: "server-logs.deploy-timeline.24h",
    auditFamily: "observability.server_logs",
  });
}
